    ...
    **Example**
    ...
    def unfreeze(self):
        if self.attr_saved is not None:
            ...
            box1 = np.asarray(self.last_measurement).flatten()
            x1, y1, s1, r1 = float(box1[0]), float(box1[1]), float(box1[2]), float(box1[3])
            w1 = np.sqrt(s1 * r1)
            h1 = np.sqrt(s1 / r1)
            box2 = np.asarray(new_history[index2]).flatten()
            x2, y2, s2, r2 = float(box2[0]), float(box2[1]), float(box2[2]), float(box2[3])
            w2 = np.sqrt(s2 * r2)
            h2 = np.sqrt(s2 / r2)
