...
def on_predict_start(predictor, persist=False):
    ...
    for i in range(predictor.dataset.bs):
        ...
        # Wrap the update method to handle Ultralytics Boxes objects
        original_update = tracker.update
        def update_wrapper(dets, im, embs=None, _original=original_update):
            if not isinstance(dets, np.ndarray):
                dets = convert_boxes_to_numpy(dets)
            return _original(dets, im, embs)
        tracker.update = update_wrapper
